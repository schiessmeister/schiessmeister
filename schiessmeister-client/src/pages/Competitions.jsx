...
const Competitions = () => {
	const { competitionsByOrganization, isLoading } = useData();
	...
	const formatDate = (dateString) => {
		...
		try {
			...
			const dateStr = dateString.split('T')[0]; // "2024-12-10"
				...
				<div className="max-w-5xl mx-auto">
					...
					<div className="mb-12">
						...
						<div className="grid gap-8 grid-cols-1 sm:grid-cols-2 md:grid-cols-3 mb-8">
							{[1, 2, 3].map((i) => (
								<Card key={i} className="rounded-xl flex flex-col gap-2">
	...
	return (
		<main className="min-h-screen w-full px-4 py-10 bg-background">
			<div className="max-w-5xl mx-auto">
				...
				{organizationGroups.length === 0 && <p className="text-center text-muted-foreground">Keine Wettbewerbe vorhanden.</p>}
								...
								{competitions.map((c) => (
									<Card key={c.id} className="rounded-xl flex flex-col gap-2">
										<CardHeader className="pb-2">
											<div className="flex items-center justify-between">
												<div className="flex items-center gap-2">
													...
													{c.isOwner && <span className="text-xs bg-green-100 text-green-800 px-2 py-0.5 rounded">Besitzer</span>}
													{!c.isOwner && c.isRecorder && <span className="text-xs bg-blue-100 text-blue-800 px-2 py-0.5 rounded">Schreiber</span>}
												</div>
												{c.isOwner && (
										...
										<CardContent className="flex flex-col gap-2">
											...
											{c.announcementUrl && (
												<a href={c.announcementUrl} target="_blank" rel="noopener noreferrer" className="text-xs text-blue-600 underline w-fit">
													Ausschreibung
												</a>
											)}
											<div className="flex justify-end w-full mt-2">
